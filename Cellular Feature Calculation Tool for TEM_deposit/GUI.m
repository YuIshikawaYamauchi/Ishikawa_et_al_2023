...
% Last Modified by GUIDE v2.5 15-Nov-2022 18:29:51
...
prm.str_class1			= get(handles.edit11,'String');
...
[ result_fd result_hole result_edge class flist ] = get_feature( fpath, outpath );
...
function show_result( flist, edges, result, filename );
	...
	for ie = 1:nx
		...
		for ii = 1:length(info(ie).id)
			...
			imax = max( img_s(:) );
			if imax > 255
				img_s = trans_16bit_to_8bit( img_s );
			end

			if length( si ) == 2
				for c = 1:3
					img_g(ys:ye,xs:xe,c) = img_s;
				end
			else
...
function [ result_fd result_hole result_edge class data ] = get_feature( fpath, outpath )
...
for i1 = 1:length( flist )
	...
	if strcmp( ext, '.tif') == 1
		...
		data(end).name_base = name;
		%if length( strfind( name, 'A' ) ) > 0
		if length( strfind( name, prm.str_class1 ) ) > 0
...
n = 0;
outpath_bw = [ outpath '/BW' ];
mkdir( outpath_bw );
for i1 = 1:length( data )
	%try
		img = imread( data(i1).filename );
		[ result_fd(i1,:) img_gray img_gray2 bw0] = cell_analize( img, prm_fd );
		[ result_hole(i1,:) img_gray img_gray2 bw1] = cell_analize( img, prm_hole );
		[ result_edge(i1,:) img_gray img_gray2 bw0] = cell_analize( img, prm_edge );

		imwrite( bw1, [ outpath_bw '/' data(i1).name_base '.png'] );
