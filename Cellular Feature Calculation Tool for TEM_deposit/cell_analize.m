function [ r img_gray img_gray2 BW ] = cell_analize( img, prm )
...
if length( si ) == 2
	img_gray = img( prm.offset.y(1)+1:end-prm.offset.y(2), prm.offset.x(1)+1:end-prm.offset.x(2));
else
	img_crop = img( prm.offset.y(1)+1:end-prm.offset.y(2), prm.offset.x(1)+1:end-prm.offset.x(2), :);
	img_gray = rgb2gray( img_crop );
end

%fprintf( 'img_gray: max=%f min=%f\n', max( img_gray(:) ), min( img_gray(:) ) );

imax = max( img_gray(:) );
if imax > 255
	img_gray = trans_16bit_to_8bit( img_gray );
end
